...
int MPI_Finalize( void )
{
    ...
    // TODO: use MPIR_Process infomation to close pbp and pop
    // associated with this process
    // printf("start modified finalize... \n"); 
    // here close pop and pbp
    POBJ_FREE(&(D_RW(externq)->data));
    POBJ_FREE(&externq);
    POBJ_FREE(&(D_RW(externm)->data));
    POBJ_FREE(&externm);

    pmemblk_close(MPIR_Process.pbp);
    pmemobj_close(MPIR_Process.pop);
    // printf("modified finalize done.\n");
    // TODO: modified above
