...
// #define PMEMblk_SIZE 8192
static long long PMEMblk_curcount = 0;


// use pmem device via filesystem
#define BUF_LEN 4096
int copy_file_to_pmem(const char *path, const char *pmempath);

// use pmem as block device
#define POOL_SIZE ((size_t)(1 << 10))
...
static int partition(int *offsets, int *counts, int *ranks, int low, int high) {
	int pivot = offsets[high];
	int i = low - 1;
	...
	swap(&offsets[i + 1], &offsets[high]);
	swap(&counts[i + 1], &counts[high]);
	swap(&ranks[i + 1], &ranks[high]);
	return (i + 1);
}


static void qsort_offset_and_count(int *offsets, int *counts, int *ranks, int low, int high) {
	if (low < high) {
		int pi = partition(offsets, counts, ranks, low, high);
		qsort_offset_and_count(offsets, counts, ranks, low, pi - 1);
		qsort_offset_and_count(offsets, counts, ranks, pi + 1, high);
	}
	return ;
}

/* request a blk from pbp, using info provideed by externq and externm 
 * returns the blkno*/
static inline int pmemblk_request() {

	int ret = -1;
    if (D_RO(externq)->head == D_RO(externq)->tail) {

        // printf("queue empty... ");
        /* queue empty: add from bitmap */
        // printf("loading queue... ");

        /* while queue not full */
        do {

            if (D_RO(D_RO(externm)->data)[D_RO(externm)->cur] != 1) {
                
                D_RW(D_RW(externq)->data)[D_RO(externq)->tail] = D_RO(externm)->cur;
                D_RW(externq)->tail++;
                if (D_RO(externq)->tail == D_RO(externq)->size) D_RW(externq)->tail = 0;

                D_RW(D_RW(externm)->data)[D_RO(externm)->cur] = 1;
            }

            D_RW(externm)->cur++;
            if (D_RO(externm)->cur == D_RO(externm)->size) D_RW(externm)->cur = 0;

        } while (!(D_RO(externq)->tail == D_RO(externq)->head - 1 || 
                    (D_RO(externq)->head == 0 && D_RO(externq)->tail == D_RO(externq)->size - 1)));
        // printf("loaded... ");
    } 
    // printf("queue not empty... ");
    ret = D_RO(D_RO(externq)->data)[D_RO(externq)->head];
    D_RW(externq)->head++;
    if (D_RO(externq)->head == D_RO(externq)->size) {
        D_RW(externq)->head = 0;
    }
    ret++;
    printf("pmemblk_request # %d\n", ret);
	return ret;
}

/* release a blk to pbp, modifying externq and externm
 * returns 0 if succeed */
static inline int pmemblk_release(int blkno) {
    /* if not full */
    blkno--;
    if (!(D_RO(externq)->tail == D_RO(externq)->head - 1 || 
            (D_RO(externq)->head == 0 && D_RO(externq)->tail == D_RO(externq)->size - 1))) {
        /* add to queue */
        D_RW(D_RW(externq)->data)[D_RO(externq)->tail] = blkno;
        D_RW(externq)->tail++;
        if (D_RO(externq)->tail == D_RO(externq)->size) D_RW(externq)->tail = 0;
    } else {
        /* edit in bitmap */
        D_RW(D_RW(externm)->data)[blkno] = 0;
    }
    printf("pmemblk_release # %d\n", blkno);
	return 0;
}
...
typedef struct ADIOI_FileD {
    ...
    char *pmem_filename;      /* edited for pmem version */
    char *pmemaddr;
    TOID(struct stat) pmem_statp;
	long long pmem_blk_count;
	TOID(int) pmem_datap;
	TOID(int) pmem_lockp;
	TOID(int) pmem_updatep;
    /* TODO: create a list of domain, of which this process take charge */
    off_t domain_off;
    size_t domain_cnt;
    int chunks;
    TOID(int) offsets;
